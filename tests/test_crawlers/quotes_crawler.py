...
class QuotesToScrapeCrawler(Crawler):
    ...
    async def login(self) -> LoggedData:
        ...
        return LoggedData(cookies=dict(self.client.cookies), headers=self.client.headers, local_storage={})
    ...
    async def process_request(self, crawler_request: CrawlerRequest) -> CrawlerResponse:
        ...
        return CrawlerResponse(url=str(response.url), text=response.text, status_code=response.status_code)
    ...
    async def parse(self, crawler_request: CrawlerRequest, crawler_response: CrawlerResponse) -> None:
        ...
        for quote in quote_list:
            data = {
                "quote": quote.css_first("span:nth-child(1)").text()[1:-1],
